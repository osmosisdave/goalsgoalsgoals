...
(function () {
  ...
  async function fetchCurrentGameweek(): Promise<CurrentGameweekResponse | null> {
    try {
      const res = await fetch(`${API_BASE_URL}/api/gameweek/current?season=${currentSeason}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      return await res.json() as CurrentGameweekResponse;
    } catch (err) {
  ...
  function statusLabel(status: string | undefined): { text: string; colour: string } {
    switch (status) {
  ...
  async function render() {
    ...
    if (token) {
      try {
        ...
        currentUsername = payload.sub ?? payload.username ?? null;
      } catch { /* ignore */ }
    }
